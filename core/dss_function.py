...
def get_element(dss, circuit, drop_element_name):
    Element = ['Line.' + ii for ii in dss.Lines.AllNames()] + ['Transformer.' + ii for ii in
...
def construct_Ymatrix(Ysparse, slack_no, totalnode_number, order_number, fast=False, path=None):
    if path is None:
        path = os.path.dirname(__file__)
    y_file = os.path.join(path, 'Ymatrix.npz')
    y_inv_file = os.path.join(path, 'Y11_inv_matrix.npz')

    # get Ymatrix
    if fast:
        Ymatrix = sparse.load_npz(y_file).todense()
        ...
        file.close()
        # save Ymatrix
        sparse.save_npz(y_file, lil_matrix(Ymatrix).tocsr())
    ...
    # get Y11_inv
    if fast:
        Y11_inv = sparse.load_npz(y_inv_file).todense()
        Y11_inv = np.array(Y11_inv)
        Y11_sparse = None
    else:
        ...
        # save Y11_inv
        sparse.save_npz(y_inv_file, lil_matrix(Y11_inv).tocsr())
...
def get_Vnode_st(dss, circuit, nodename):
    circuit.SetActiveBus(nodename)
    voltage = dss.Bus.VMagAngle()
    v_ph = voltage[0] / (14.399 * 1000)
    v_ang = voltage[1]
    #    Vmag = [ii / dss.Bus.kVBase() / 1000 for ii in voltage[0:len(voltage):2]]
    #    allbusnode = dss.Bus.Nodes()
    #    phase = nodename.split('.')[1]
...
def getElemCurrents_1ph(circuit, dss, type, name):
    circuit.SetActiveElement(type + '.' + name)
    s = dss.CktElement.CurrentsMagAng()
    Imag = s[0]
    Iang = s[1]
...
def get1phLinePower(circuit, dss, type, name):
    circuit.SetActiveElement(type + '.' + name)
    s = dss.CktElement.Powers()
    lens = int(len(s) / 2)
    powers = [sum(s[0:lens:2]), sum(s[1:lens:2])]
...
def get_Ymatrix(dss, circuit, feeder_directory, show_y=True):
    # ------- get Ymatrix file ------------
...
def system_topology_matrix_form(dss, AllNodeNames, slack_number, Ynode_file, drop_element_name, **kwargs):
    ...
    Ysparse_file = os.path.join(os.getcwd(), str(dss.Circuit.Name()).upper() + '_SystemY.txt')
    Yinformation = construct_Ymatrix(Ysparse_file, slack_number, node_number, order_nodes, **kwargs)
    print('Finish getting Ybus information')

    return Yinformation


def get_node_information(dss):
    AllNodeNames = dss.Circuit.YNodeOrder()
    # --------- get Voltage BasekV Information ----------
    node_number = len(AllNodeNames)
    ...
    return AllNodeNames, Vbase_allnode, node_number
